...
function addWorker(build, worker) {
  ...
  function onWorkerCommand(cmd, args, options) {
    console.log('WORKKER COMMAND %j'.red, cmd);
    var command = {
      command: cmd,
      args: args,
      options: options,
  ...
  function onWorkerStdout(buf) {
    console.log('WORKKER STDOUT %j'.red, buf);
    if (build.lastCommand) {
  ...
  function onWorkerStderr(buf) {
    console.log('WORKKER STDERR %j'.red, buf);
    if (build.lastCommand) {
      build.lastCommand.out += buf;
  ...
  function onWorkerClose(code) {
    console.log('WORKKER CLOSED %j'.red, code);
    if (build.lastCommand) {
  ...
  function onBuildEnd() {
    console.log('[HUB] build ended'.red);
    build.ended = true;
