...
function work(build, cb) {
  ...
  function run() {
    if (build.previous_successful_build)
      Builds.get(build.project, build.previous_successful_build, gotBuild);
    else
      startWorker();
  }

  function gotBuild(err, build) {
    if (err) cb(err);
    else {
      previousBuild = build;
      startWorker();
  ...
  function runStage(stage, cb) {
    ...
    if ('function' == typeof stage) {
      stage(build, worker, previousBuild);
    } else {
      Plugins.each(plugins, stage, build, worker, previousBuild);
    }
