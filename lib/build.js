...
function work(build, cb) {
  ...
  function loadPreviousBuild(cb) {
    if (build.previous_successful_build)
      Build.findOne({id:build.previous_successful_build}, cb);
    else cb();
  }
  ...
  function maybeStartWorker() {
    ...
    if (start) {
      ...
      build.state = 'pending';
      console.log('before save build rev:', build.rev);
      build.save(updatedBuild);
    }
  }

  function updatedBuild(err) {
    console.log('after save build rev:', build.rev);
    if (err) cb(err);
    else startWorker();
  }
