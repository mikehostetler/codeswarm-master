...
function work(build, cb) {
  ...
  function run() {
    async.parallel([
      loadBuild,
      loadPreviousBuild], initialized);
  }

  function initialized(err, results) {
    if (err) cb(err);
    else {
      console.log('INITIALIZED RESULTS:', results);
      build = results[0];
      previousBuild = results[1];
      startWorker();
    }
  }

  function loadBuild(cb) {
    Build.findOne({id: build.id}, cb);
  }

  function loadPreviousBuild(cb) {
    var buildId = build.previous_successful_build;
    if (! buildId) cb();
    else Build.findOne({id: buildId }, cb);
  }
