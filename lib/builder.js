							...
							if (err) {
								...
								repo.commits(build.branch, 1, function (err, commits) {
									if (err) {
										...
										build.state.commit = commits[0];
										trace(build, "├ Commit:   " + commits[0].id);
										trace(build, "├ Message:  " + commits[0].message);
										trace(build, "└ Author:   " + commits[0].committer.name + " &lt;" + commits[0].committer.email + "&gt;");
										callback(null);
									}
								});
							}
