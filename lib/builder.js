...
var trace = function (build, data) {
	...
	// Build active log
	if (active_log.hasOwnProperty[build.state.id]) {
		active_log[build.state.id] += data + "\n";
	} else {
		active_log[build.state.id] = data + "\n";
	}
	...
	if (build.state.status === "fail") {
		...
		// Remove active log entry
		delete active_log[build.state.id];
