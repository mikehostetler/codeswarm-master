...
var configuration = require("./configuration.js"),
	...
	expressRunner = require("./express-runner.js")(),
...
startExpress = function (build, build_dir, callback) {
	if (!_.isNull(build.state.config.express.script) && build.state.config.express.script !== "") {
		expressRunner.add(build_dir, function (err) {
			callback(err);
		});
	} else {
		callback(null);
...
builder = function (build) {
	...
	async.series({
		...
		clone: function (callback) {
			...
			git.clone(build.repo, config.app.temp + build.state.cwd, function (err, repo) {
				if (err) {
					callback(err);
				} else {
		...
		config: function (callback) {
			...
			fs.readFile(config.app.temp + build.state.cwd + "/.vouch.json", function (err, config) {
				if (err) {
					callback(err);
				} else {
		...
		if (err) {
			...
			config.projects[build._id] = build;
			configuration.set(config);
			// Remove temp folder
			fse.remove(config.app.temp + build.state.cwd);
		} else {
			// Set build status
			build.state.status = "pass";
			trace(build, "BUILD COMPLETE");
			// Set build status in logs index
			fs.appendFileSync(config.app.logs + build._id + "/index", "\n" + build.state.id + "-" + build.state.status);
			config.projects[build._id] = build;
			configuration.set(config);
			// Copy to builds
			fse.copy(config.app.temp + build.state.cwd, config.app.builds + build._id, function (err) {
				if (err) {
					console.log(err);
				}
				// Remove temp folder
				fse.remove(config.app.temp + build.state.cwd);
				startExpress(build, config.app.builds + build._id, function () {});
			});
