...
exports.repoDeployKeys = repoDeployKeys;

function repoDeployKeys(repo, token, cb) {

  var keys = [];

  function _repoDeployKeys(page) {
    var options = {
      headers: {
        'Authorization': 'token ' + (token.token || token),
        'User-Agent': 'codeswarm,request,node.js'
      },
      uri: 'https://api.github.com/repos/' + repo + '/keys',
      qs: {
        page: page,
        per_page: 100
      },
    ...
    request(options, gotDeployKeys);

    function gotDeployKeys(err, res, _keys) {
      if (err) return cb(err);
      if (res.statusCode != 200)
        return cb(new Error('Github API call returned status code ' + res.statusCode));
      ...
      if (_keys && _keys.length == 100) {
        _repoDeployKeys(page + 1);
      } else cb(null, keys);
    }
  }
...
function createDeployKey(repo, title, key, token, cb) {
  var options = {
    headers: {
      'Authorization': 'token ' + (token.token || token),
      'User-Agent': 'codeswarm,request,node.js'
  ...
  request.post(options, replied);

  function replied(err, res, body) {
    if (! err && res.statusCode != 201)
      err = new Error('Github reply was ' + res.statusCode + ': ' + JSON.stringify(body));
    cb(err);
  }
}